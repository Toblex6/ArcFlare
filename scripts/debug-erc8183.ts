...
async function main() {
  ...
  const createTx = await circleClient.createContractExecutionTransaction({
    ...
    abiParameters: [PROVIDER_ADDRESS, CLIENT_ADDRESS, expiredAt.toString(), "Debug test job", "0x0000000000000000000000000000000000000000"],
  ...
  if (receipt.logs.length === 0) {
    console.log("   ❌ No logs found in the receipt. The transaction may have succeeded but the contract didn't emit the event.");
  } else {
    for (const [index, log] of receipt.logs.entries()) {
      ...
      if (log.address.toLowerCase() === AGENTIC_COMMERCE_CONTRACT.toLowerCase()) {
        ...
        const jobCreatedEventSignature = "0x957e1da045e5f970beaca1ebf7de3f62f6ec093512dac48ffb2e94e8bb4aa5c7";

 ...
 *   B5  no settlement/transfer mechanism was touched (x402/CCTP/payroll/nano
 *       remain USDC-only), token table not duplicated.
...
async function main() {
  ...
  console.log("\n[B5] USDC-only settlement mechanisms untouched (Phase 2 excluded)");
  const changed = execSync("git diff --name-only", { cwd: root, encoding: "utf8" }).split(/\r?\n/).filter(Boolean);
  const protectedPaths = [
    "src/app/api/payments/settle",
    "src/lib/x402.ts",
    "src/app/api/x402",
    "src/lib/payroll",
    "src/lib/circle",

...
export default function EscrowDashboard() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f8fafc", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      {/* Sidebar */}
      <aside style={{ width: 220, minHeight: "100vh", background: "#1f140f", display: "flex", flexDirection: "column", padding: "24px 14px", flexShrink: 0, position: "sticky", top: 0, height: "100vh", overflowY: "auto" }}>
        ...
        <nav style={{ display: "flex", flexDirection: "column", gap: 3, flex: 1 }}>
          {NAV.map((item) => (
            <a key={item.label} href={item.href} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 9, textDecoration: "none", fontSize: 13, fontWeight: 500, background: (item as any).active ? "rgba(13,124,95,0.18)" : "transparent", color: (item as any).active ? "#0d7c5f" : "#6b7280", border: (item as any).active ? "1px solid rgba(13,124,95,0.25)" : "1px solid transparent" }}>
              {item.label}
            </a>
          ))}
        </nav>
      ...
      <main style={{ flex: 1, padding: "32px 32px", overflowX: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: "#0f172a", margin: "0 0 4px 0" }}>Escrow Management</h1>
            <p style={{ fontSize: 13, color: "#64748b", margin: 0 }}>Trustless USDC escrow on Arc Testnet via ArcFlareEscrow contract</p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(13,124,95,0.08)", border: "1px solid rgba(13,124,95,0.2)", borderRadius: 20, padding: "5px 14px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }}>
            {[
              ...
              <div key={i} style={{ background: m.bg, border: `1px solid ${m.border}`, borderRadius: 14, padding: 18 }}>
                <p style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8, margin: "0 0 8px 0" }}>{m.label}</p>
                <p style={{ fontSize: 22, fontWeight: 700, color: m.color, marginBottom: 4, fontFamily: "monospace", margin: "0 0 4px 0" }}>{m.value}</p>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          ...
          <button onClick={fetchEscrows} style={{ marginLeft: "auto", padding: "6px 14px", borderRadius: 8, fontSize: 12, cursor: "pointer", border: "1px solid #e2e8f0", background: "#fff", color: "#64748b" }}>↻ Refresh</button>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: selected ? "1fr 380px" : "1fr", gap: 20 }}>
          <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: 24 }}>
            ...
            {escrows.length === 0 ? (
              <div style={{ textAlign: "center", padding: "48px 0" }}>
                ...
                <p style={{ color: "#cbd5e1", fontSize: 12, marginTop: 4 }}>Create an escrow via POST /api/escrow/create</p>
              </div>
            ) : (
                  ...
                  <thead>
                    <tr style={{ borderBottom: "1px solid #f1f5f9" }}>
                      {["Reference", "Parties", "Amount", "Condition", "Deadline", "Status", "Confirmations"].map((h) => (
                        <th key={h} style={{ textAlign: "left", paddingBottom: 12, paddingRight: 14, fontSize: 10, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.5, fontWeight: 500 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {escrows.map((e) => {
                      ...
                      return (
                        <tr key={e.id} className="hover-row" style={{ borderBottom: "1px solid #f8fafc" }} onClick={() => setSelected(selected?.id === e.id ? null : e)}>
                          ...
                          <td style={{ padding: "14px 14px 14px 0" }}>
                            {e.deadline ? (
                              <>
                                <div style={{ color: e.isExpired ? "#dc2626" : "#0f172a", fontSize: 11 }}>{e.isExpired ? "⚠ Expired" : `⏱ ${formatTime(e.timeRemaining || 0)}`}</div>
                          ...
                          <td style={{ padding: "14px 14px 14px 0" }}>
                            <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: 10, fontWeight: 700, background: sc.bg, color: sc.text, border: `1px solid ${sc.border}` }}>{e.status}</span>
                          </td>
          ...
          {/* Detail Panel */}
          {selected && (
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: 24, alignSelf: "start" }}>
              ...
              {(() => {
                ...
                return <span style={{ padding: "4px 12px", borderRadius: 20, fontSize: 11, fontWeight: 700, background: sc.bg, color: sc.text, border: `1px solid ${sc.border}` }}>● {selected.status}</span>;
              })()}
              <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  ...
                  <div key={row.label} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 8, padding: 12 }}>
                    <p style={{ fontSize: 10, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4, margin: "0 0 4px 0" }}>{row.label}</p>
                    <p style={{ fontSize: 12, color: "#0f172a", fontFamily: "monospace", wordBreak: "break-all", margin: 0 }}>{row.value}</p>
                  </div>
                ))}
                <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 8, padding: 12 }}>
                  <p style={{ fontSize: 10, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8, margin: "0 0 8px 0" }}>Confirmations</p>
                  <div style={{ display: "flex", gap: 10 }}>
                    {[{ label: "Depositor", confirmed: selected.depositorConfirmed }, { label: "Beneficiary", confirmed: selected.beneficiaryConfirmed }].map((c) => (
                      <div key={c.label} style={{ flex: 1, textAlign: "center", padding: 8, background: c.confirmed ? "#f0fdf4" : "#f8fafc", borderRadius: 8, border: `1px solid ${c.confirmed ? "#bbf7d0" : "#e2e8f0"}` }}>
                        <p style={{ fontSize: 16, marginBottom: 4, margin: "0 0 4px 0" }}>{c.confirmed ? "✅" : "⏳"}</p>
                ...
                {selected.disputeReason && (
                  <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 8, padding: 12 }}>
                    <p style={{ fontSize: 10, color: "#d97706", marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5, margin: "0 0 4px 0" }}>Dispute Reason</p>
                    <p style={{ fontSize: 12, color: "#0f172a", margin: 0 }}>{selected.disputeReason}</p>
                    {selected.disputedBy && <p style={{ fontSize: 10, color: "#94a3b8", marginTop: 4, margin: "4px 0 0 0" }}>Raised by: {selected.disputedBy}</p>}
                  </div>
                )}
              </div>
            </div>

...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!payment) return NextResponse.json({ success: false, error: "Invoice node not found." }, { status: 404 });
    ...
    return NextResponse.json({
      ...
      payment: {
        ...
        businessName: payment.merchant, // Use the string field directly
        agentSCA: payment.agentSCA,
        txHash: payment.arcTxHash, // Assuming arcTxHash is txHash
      },
    });
  } catch (error) {
    console.error("Database mapping read failure:", error);
    return NextResponse.json({ success: false, error: "Database mapping read failure." }, { status: 500 });
  }
}
...
export default function CheckoutPage() {
  ...
  if (error || !payment) {
    return (
      <main style={{ minHeight: "100vh", background: "#0e0b08", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ background: "#1a1410", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 24, padding: 40, maxWidth: 400, textAlign: "center" }}>
          <p style={{ color: "#f87171", fontWeight: 700, marginBottom: 8 }}>Ledger Disconnect</p>
          <p style={{ color: "#6b5a45", fontSize: 13 }}>{error || "The reference could not be found."}</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#0e0b08", color: "#f0ece6", fontFamily: "Inter, system-ui, sans-serif", padding: "32px 24px" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
        ...
        <div style={{ background: "#1a1410", border: "1px solid #2d2015", borderRadius: 24, padding: 32 }}>
          <p style={{ color: "#c8975a", fontSize: 10, textTransform: "uppercase", letterSpacing: 2, fontFamily: "monospace", margin: "0 0 8px" }}>Hosted Checkout</p>
          <h2 style={{ fontSize: 28, fontWeight: 800, color: "#f0ece6", margin: "0 0 28px", lineHeight: 1.2 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 28 }}>
            {[
              ...
              <div key={i} style={{ background: "#251c12", border: "1px solid #3d2e1a", borderRadius: 14, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span style={{
                  ...
                  {row.highlight && <span style={{ color: "#c8975a", fontSize: 14, marginLeft: 6 }}>{payment.currency}</span>}
                </span>
              </div>
            ))}
            ...
            <div style={{ background: "#251c12", border: "1px solid #3d2e1a", borderRadius: 14, padding: "14px 18px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: agent ? 8 : 0 }}>
                ...
                {agent && (
                  <span style={{ fontSize: 10, color: "#c8975a", fontFamily: "monospace", background: "rgba(200,151,90,0.1)", border: "1px solid rgba(200,151,90,0.2)", padding: "2px 8px", borderRadius: 10 }}>
                    ERC-8004 #{agent.tokenId}
                  </span>
                )}
              </div>
              <p style={{ color: "#f0ece6", fontSize: 12, fontFamily: "monospace", wordBreak: "break-all", margin: "4px 0 0" }}>
                {displayAddress}
              </p>
              {agent && (
          ...
          {isConfirmed && (
            <div style={{ marginTop: 16, background: "rgba(6,182,212,0.06)", border: "1px solid rgba(6,182,212,0.15)", borderRadius: 14, padding: 16, textAlign: "center" }}>
              ...
              <p style={{ color: "#4b4035", fontSize: 11, margin: 0 }}>Ledger updated · Dashboard synced</p>
            </div>
          )}
        </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                ...
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <span style={{ color: row.color, fontSize: 11, fontFamily: "monospace" }}>{row.value}</span>
                </div>
              ))}
            </div>
          </div>

...
export async function POST(request: Request) {
  try {
    ...
    // M2: port of the cctp-settle guards — unique source-message claim
    // (P2002 → 409) + decodeBurnMessage recipient/amount binding against the
    // DB row's merchantSCA/amount. Claim released on binding failure.
    try {
      await prisma.paymentLog.update({
        where: { reference },
    ...
    }
    {
      ...
      const { mintRecipient, amount: burnedAmount } = decodeBurnMessage(rawMessage);
      const expectedRecipient = String((paymentRow as any).merchantSCA ?? '').toLowerCase();
      const expectedAmount = parseUnits(Number((paymentRow as any).amount).toString(), 6);
      if (!expectedRecipient || mintRecipient.toLowerCase() !== expectedRecipient) {
        ...
        return NextResponse.json(
          { success: false, error: 'This transaction does not pay the correct recipient for this payment.' },
          { status: 400 }
        );
      }

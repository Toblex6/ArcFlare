...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const handler = async (innerReq: NextRequest) => {
    ...
    const { clientWalletId, description, criteria, budget, evaluatorAddress } = body;
    ...
    return NextResponse.json({ success: true, jobId: jobId.toString(), dbId: job.id, txHash, status: "OPEN", agent: { id: agent.id, tokenId: agent.tokenId, name: agent.name, scaAddress: agent.scaAddress }, criteriaHash, nextSteps: { setBudget: { endpoint: "/api/jobs/set-budget", body: { jobId: jobId.toString(), providerWalletId: agent.circleWalletId, budget: budgetBigInt.toString() } }, fund: { endpoint: "/api/jobs/fund", body: { jobId: jobId.toString(), clientWalletId } } } });
  };

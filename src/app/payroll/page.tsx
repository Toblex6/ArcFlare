...
export default function PayrollPage() {
  ...
  const addRecipient = () => setRecipients([...recipients, { recipientSCA: "", amount: "", label: "" }]);
  ...
  const runPayroll = async () => {
    ...
    try {
      ...
      if (validRecipients.length === 0) throw new Error("Add at least one recipient with an address and amount.");
  ...
  const lookupBatch = async () => {
    ...
    try {
      const res = await fetch(`/api/payroll/run?batchRef=${lookupRef}`, { headers: { "x-api-key": API_KEY } });
      const data = await res.json();
      if (!data.success) throw new Error(data.error);
  ...
  const S = {
    ...
    input:   { width: "100%", padding: "10px 14px", background: "#251c12", border: "1px solid #3d2e1a", borderRadius: 10, color: "#f0ece6", fontSize: 13, fontFamily: "monospace", outline: "none", boxSizing: "border-box" as const },
    ...
    <div style={S.page}>
      ...
      <main style={S.main}>
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#f0ece6", margin: "0 0 4px" }}>Batch Payroll</h1>
          <p style={{ color: "#6b5a45", fontSize: 13, margin: 0 }}>Pay any number of recipients in a single onchain batch</p>
        </div>
        ...
        {activeTab === "run" && (
          <div style={S.card}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
              ...
              <div><span style={S.label}>Payer Circle Wallet ID</span><input style={S.input} value={payerWalletId} onChange={(e) => setPayerWalletId(e.target.value)} /></div>
            </div>

            <p style={{ color: "#c8975a", fontSize: 12, fontWeight: 700, margin: "0 0 12px", textTransform: "uppercase" as const, letterSpacing: 1 }}>Recipients</p>

            {recipients.map((r, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr auto", gap: 8, marginBottom: 8, alignItems: "center" }}>
                <input style={S.input} value={r.recipientSCA} onChange={(e) => updateRecipient(i, "recipientSCA", e.target.value)} placeholder="0xRecipientAddress" />
                <input style={S.input} value={r.amount} onChange={(e) => updateRecipient(i, "amount", e.target.value)} placeholder="Amount USDC" />
                <input style={S.input} value={r.label} onChange={(e) => updateRecipient(i, "label", e.target.value)} placeholder="EMP-001" />
                <button onClick={() => removeRecipient(i)} style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 8, color: "#f87171", padding: "10px 12px", cursor: "pointer", fontSize: 12 }}>✕</button>
              </div>
            ))}

            <button onClick={addRecipient} style={{ marginTop: 8, marginBottom: 20, padding: "8px 16px", background: "transparent", border: "1px dashed #3d2e1a", borderRadius: 8, color: "#6b5a45", fontSize: 12, cursor: "pointer" }}>
              + Add Recipient
            </button>

            <div style={{ background: "rgba(200,151,90,0.06)", border: "1px solid rgba(200,151,90,0.2)", borderRadius: 10, padding: "12px 16px", marginBottom: 16, display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#8a7560", fontSize: 13 }}>Total Payroll</span>
              <span style={{ color: "#c8975a", fontWeight: 700, fontFamily: "monospace" }}>{totalAmount.toFixed(6)} USDC — {recipients.filter(r=>r.recipientSCA && r.amount).length} recipients</span>
            </div>
            ...
            {runError && <p style={{ color: "#f87171", fontSize: 12, marginTop: 10 }}>❌ {runError}</p>}

            {runResult && (
              <div style={{ marginTop: 16, background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 14, padding: 20 }}>
                <p style={{ color: "#10b981", fontWeight: 700, fontSize: 14, margin: "0 0 4px" }}>{runResult.status}</p>
                <p style={{ color: "#f0ece6", fontSize: 12, margin: "0 0 14px" }}>{runResult.message}</p>
                {runResult.results.map((r: any, i: number) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #2d2015" }}>
                    <span style={{ color: "#f0ece6", fontSize: 11, fontFamily: "monospace" }}>{r.recipientSCA.slice(0,10)}... {r.label ? `(${r.label})` : ""}</span>
                    <span style={{ color: r.status === "SUCCESS" ? "#10b981" : "#f87171", fontSize: 11, fontWeight: 700 }}>{r.status === "SUCCESS" ? `✅ ${r.amount} USDC` : "❌ Failed"}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {activeTab === "lookup" && (
          <div style={S.card}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: "#f0ece6", margin: "0 0 16px" }}>Look Up Payroll Batch</h3>
            <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
              <input style={{ ...S.input, flex: 1 }} value={lookupRef} onChange={(e) => setLookupRef(e.target.value)} placeholder="payroll_xxx" />
            ...
            {lookupResult && (
              <pre style={{ color: "#f0ece6", fontSize: 11, fontFamily: "monospace", whiteSpace: "pre-wrap" as const, background: "#251c12", borderRadius: 10, padding: 16 }}>

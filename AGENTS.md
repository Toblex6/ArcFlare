...
- **Live E2E (funds move — tiny amounts, controlled wallets only)**: `npx tsx scripts/unitflow-e2e-usdc-eurc.ts` needs `ARC_TESTNET_RPC`, `RELAYER_PRIVATE_KEY` (payer/signer `0x0d9Dc173…`), `SELLER_ADDRESS` (merchant `0xc119bb61…`, no key — recipient never signs). Amount is 0.01 USDC; never raise it without review. RPC flakes are environmental — retry across the primary + alternates before calling a failure real.
